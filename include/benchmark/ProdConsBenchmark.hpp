...
public:
    ProdConsBenchmark(     size_t prodCount,
                        size_t consCount,
                        double additional,
                        bool balanced,
    ...
    void run(const seconds runDuration, const size_t numRuns,std::string fileName=""){
                ...
                for(size_t nCons: consumerSet) {
                    for(size_t queueSize : sizeSet) {
                        for(size_t warmup : warmupSet){
                            ProdConsBenchmark bench(nProd,nCons,additionalWork,balancedLoad,queueSize,warmup,args);
