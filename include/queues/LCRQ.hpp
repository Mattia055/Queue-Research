...
public:
    CRQueue(uint64_t start, size_t Ring_Size=RING_SIZE):
    Base(),
    Ring_Size{Ring_Size}
    {
    ...
    static std::string className() {
        ...
        return "CRQueue"s + (padded_cells ? "/padded":"");
    }

    bool enqueue(T* item, [[maybe_unused]] int tid) {
        int try_close = 0;
...
#endif
using LCRQueue = LinkedRingQueue<T, CRQueue<T, padded_cells>>;