...
PADDED_PATH = "build/paddedMain"
UNPADDED_PATH = "build/unpaddedMain"
DELIM = ","
NULL_ARG = "NULL"
...
QUEUES_HASH = {
    "BoundedMTQ": "BoundedMTQueue",
    "BMTQ"      : "BoundedMTQueue",
    "BMTQueue" : "BoundedMTQueue",
    "BoundedMTQueue": "BoundedMTQueue",
...
BOUNDED     : set = {"BoundedCRQueue", "BoundedPRQueue", "BoundedMuxQueue","BoundedMTQueue"}
...
REQUIRED            = {"benchmark","path","threads","runs","iterations","queues"}
REQUIRED_PROD_CONS  = {"threads","producers","consumers"} #if thread not set
REQUIRED_MEM        = {"benchmark","path","producers","consumers","queues","duration_sec","granularity_msec"}
OPTIONAL            = {"warmup","additionalWork","flags","sizes","ratio"}
#union to optional
...
def parseBenchmark(benchmark:dict) -> list :
    ...
    if id == "Pairs":
        required = REQUIRED.union(REQUIRED_PROD_CONS)
        optional = optional.union(OPTIONAL_PROD_CONS)
    #     threads, producers, consumers = map(benchmark.get, ["threads", "producers", "consumers"])
    #     if(benchmark.get("threads") is not None and benchmark.get("ratio") is not None): #ratio casting
    #         required = required.difference("producers","consumers")
    #     elif benchmark.get("threads") is not None and benchmark.get("ratio") is None: #casting produttori
    ...
    for key in required:
        if benchmark.get(key) is None: continue #strerror(f"Missing Required Key: {key}")
        match key:
            case "runs" | "iterations" | "granularity_msec" | "duration_sec":
                arg = benchmark[key]
                if type(arg) is not int or arg <= 0:
                    strerror(f"Invalid {key} value: {arg}")
                benchmark[key] = str(arg)
            case "threads" |"producers"|"consumers":
                arg = benchmark[key]
    ...
    for key in optional:
        if key != "balanced" and benchmark.get(key) is None:
            benchmark[key] = NULL_ARG
        else:
            match key:
                case "ratio":
                    if type(benchmark[key]) is not list:
                        strerror(f"Invalid {key} value: {benchmark[key]}")
                    if len(benchmark[key]) != 2:
                        strerror(f"Invalid {key} value: {benchmark[key]}")
                    if any(type(i) is not int or i <= 0 for i in benchmark[key]):
                        strerror(f"Invalid {key} value: {benchmark[key]}")
                    benchmark[key] = DELIM.join(list(map(str,benchmark[key])))
                case "flags":
                ...
                case "additionalWork":
                    ...
                    benchmark[key] = DELIM.join(list(map(str,arg)))
                case "balanced":
                    balanced = benchmark.get(key)
                    if balanced is None: benchmark[key] = "0"
                    elif type(benchmark.get(key)) is not bool:
                        strerror(f"Invalid {key} value: {benchmark.get(key)}")
                        benchmark[key] = "1" if benchmark.get(key) else "0"

    return [f"{arg}:{benchmark[arg]}" for arg in benchmark.keys()]
...
if __name__ == "__main__":
    ...
    if args.json_file is None:
        ...
        positional.add_argument ("path", type=str, help="(Required) output path")
        ...
        execCtrl.add_argument   ("-r","--runs",help="(Required) Repetition of the same test",type=int,metavar="")
        execCtrl.add_argument   ("-i","--iter",help="(Required) Iteration|Duration",type=int,metavar="")
